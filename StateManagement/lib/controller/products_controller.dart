...
class ProductController extends GetxController {
  ...
  void fetchProduct() async {
    ...
    // From ProducsAPI
    var fetchProductsFromJson =
        await http.get(Uri.parse('https://dummyjson.com/products'));

    var productsJson = json.decode(fetchProductsFromJson.body);

    var productsList = <Product>[];

    for (int i = 0; i < productsJson['products'].length; i++) {
      var item = productsJson['products'][i];

      // Fetch images
      List imagesList = [];
      for (var j = 0; j < item['images'].length; j++) {
        imagesList.add(item['images'][j]);
      }
